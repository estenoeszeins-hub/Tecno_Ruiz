/*
 * -----------------------------------------------------------------
 * EL MOTOR DE AULITA VIRTUAL (v6 - Sin DOMContentLoaded)
 * -----------------------------------------------------------------
 * Este script se ejecuta inmediatamente al ser cargado,
 * ya que el 'HTML Adicional' de Moodle controla el timing.
 * -----------------------------------------------------------------
 */
...
// El código ahora se ejecuta al nivel superior
...
function addImageThumbnail(src) {
    const fullImagePath = CHATBOT_BASE_PATH + "/" + src;
    const img = document.createElement('img');
    img.className = 'chat-image-thumbnail';
    img.src = fullImagePath;
    img.dataset.src = fullImagePath;
...
optionsContainer.addEventListener('click', (e) => {
    if (e.target.classList.contains('option-button')) {
        ...
        addUserMessage(text);
        logToSheet("Opcion", text, nextStep);
...
function handleTextInput() {
    ...
    if (currentStep === 'validar_docente') {
        ...
        if (text === TEACHER_PASSWORD) {
            ...
            logToSheet("Acceso Docente", "Éxito", stepConfig.siguiente_paso);
            inputField.value = '';
            processStep(stepConfig.siguiente_paso);
        } else {
            logToSheet("Acceso Docente", "Fallo de clave", stepConfig.paso_fallido);
            inputField.value = '';
    ...
    logToSheet(stepConfig.variable || "Texto", text, stepConfig.siguiente_paso);
...
// --- 7. FUNCIÓN DE LOG A GOOGLE SHEETS ---
function logToSheet(dataType, value, nextStep) {
    
    if (GOOGLE_SCRIPT_URL === "https://estenoeszeins-hub.github.io/Tecno_Ruiz/GOOGLE_SCRIPT_URL_DEBES_PEGARLA_AQUI") {
